...
void RGB_Init(void)
{
    ...
    led_strip_config_t strip_config = {
        ...
        .max_leds = BLINK_MAX_leds, // at least one LED on board
    };
...
void Set_RGB(uint8_t ledx, uint8_t red_val, uint8_t green_val, uint8_t blue_val)
{
    ...
    led_strip_set_pixel(led_strip, ledx, red_val, green_val, blue_val);
...
void _RGB_Example(void *arg)
{
    ...
    while(1)
    {
        for(int j = 0; j < BLINK_MAX_leds; j++){
            Set_RGB(j,RGB_Data[i][0]*3,RGB_Data[i][1]*3,RGB_Data[i][2]*3);
        }
        i++;
        if(i >= 192) i = 0;
